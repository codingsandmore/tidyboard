...
func TestWalletService_CreditWallet_AppendsTxAndAdjustsBalance(t *testing.T) {
	...
	_, err := pool.Exec(ctx,
		`INSERT INTO households (id, name, timezone, invite_code, created_by_account_id) VALUES ($1, 'Test', 'UTC', 'WTEST1', $2)`,
		householdID, uuid.New(),
	)
	...
	_, err = pool.Exec(ctx,
		...
		memberAccountID, "wallet-test-1@example.com",
	)
	require.NoError(t, err)

	_, err = pool.Exec(ctx,
		`INSERT INTO members (id, household_id, account_id, display_name, role) VALUES ($1, $2, $3, 'Kid1', 'child')`,
...
func TestWalletService_Credit_Negative_DoesNotPanic(t *testing.T) {
	...
	_, err := pool.Exec(ctx,
		`INSERT INTO households (id, name, timezone, invite_code, created_by_account_id) VALUES ($1, 'Test2', 'UTC', 'WTEST2', $2)`,
		householdID, uuid.New(),
	)
	require.NoError(t, err)
	...
	_, err = pool.Exec(ctx,
		`INSERT INTO accounts (id, email, password_hash) VALUES ($1, $2, 'x') ON CONFLICT DO NOTHING`,
		memberAccountID, "wallet-test-2@example.com",
	)
	require.NoError(t, err)

	_, err = pool.Exec(ctx,
		`INSERT INTO members (id, household_id, account_id, display_name, role) VALUES ($1, $2, $3, 'Kid2', 'child')`,
		memberID, householdID, memberAccountID,
	)
	require.NoError(t, err)
