...
func (h *RoutineHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to list routines")
		return
	}
...
func (h *RoutineHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to create routine")
		return
	}
...
func (h *RoutineHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusBadRequest, "bad_request", "invalid routine ID")
		return
	}
	...
	if err != nil {
		...
		default:
			respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to update routine")
		}
		return
	}
...
func (h *RoutineHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.svc.Delete(r.Context(), householdID, id); err != nil {
		...
		case service.ErrNotFound:
			respond.Error(w, http.StatusNotFound, "not_found", "routine not found")
		default:
			respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to delete routine")
		}
		return
	}
...
func (h *RoutineHandler) AddStep(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case service.ErrNotFound:
			respond.Error(w, http.StatusNotFound, "not_found", "routine not found")
		default:
			respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to add step")
		}
		return
	}
...
func (h *RoutineHandler) UpdateStep(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusBadRequest, "bad_request", "invalid step ID")
		return
	}
	...
	if err != nil {
		switch err {
		case service.ErrNotFound:
			respond.Error(w, http.StatusNotFound, "not_found", "step not found")
		default:
			respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to update step")
		}
		return
	}
...
func (h *RoutineHandler) DeleteStep(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.svc.DeleteStep(r.Context(), householdID, routineID, stepID); err != nil {
		...
		case service.ErrNotFound:
			respond.Error(w, http.StatusNotFound, "not_found", "step not found")
		default:
			respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to delete step")
		}
		return
	}
	w.WriteHeader(http.StatusNoContent)
...
func (h *RoutineHandler) MarkComplete(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case service.ErrNotFound:
			respond.Error(w, http.StatusNotFound, "not_found", "routine not found")
		default:
			respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to mark completion")
		}
		return
	}
...
func (h *RoutineHandler) UnmarkCompletion(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusBadRequest, "bad_request", "invalid completion ID")
		return
	}
	...
	if err := h.svc.UnmarkCompletion(r.Context(), householdID, routineID, completionID); err != nil {
		...
		case service.ErrNotFound:
			respond.Error(w, http.StatusNotFound, "not_found", "routine not found")
		default:
			respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to unmark completion")
		}
		return
	}
	w.WriteHeader(http.StatusNoContent)
...
func (h *RoutineHandler) GetStreak(w http.ResponseWriter, r *http.Request) {
	...
	if memberIDStr == "" {
		respond.Error(w, http.StatusBadRequest, "validation_error", "member_id query param is required")
		return
	}
	...
	if err != nil {
		...
		case service.ErrNotFound:
			respond.Error(w, http.StatusNotFound, "not_found", "routine not found")
		default:
			respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to get streak")
		}
		return
	}
...
func (h *RoutineHandler) ListCompletionsForDay(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusBadRequest, "bad_request", "invalid date format, use YYYY-MM-DD")
		return
	}
