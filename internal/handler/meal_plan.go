...
func (h *MealPlanHandler) Upsert(w http.ResponseWriter, r *http.Request) {
	...
	if req.Date == "" {
		respond.Error(w, http.StatusBadRequest, "validation_error", "date is required")
		return
	}
	validSlots := map[string]bool{"breakfast": true, "lunch": true, "dinner": true, "snack": true}
	if !validSlots[req.Slot] {
		respond.Error(w, http.StatusBadRequest, "validation_error", "slot must be breakfast, lunch, dinner, or snack")
		return
	}

	entry, err := h.svc.Upsert(r.Context(), householdID, req)
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to upsert meal plan entry")
		return
	}
...
func (h *MealPlanHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if from == "" || to == "" {
		respond.Error(w, http.StatusBadRequest, "validation_error", "from and to query params are required (YYYY-MM-DD)")
		return
	}

	entries, err := h.svc.List(r.Context(), householdID, from, to)
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to list meal plan entries")
		return
	}
...
func (h *MealPlanHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusBadRequest, "bad_request", "invalid meal plan entry ID")
		return
	}

	if err := h.svc.Delete(r.Context(), householdID, id); err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "failed to delete meal plan entry")
		return
	}
