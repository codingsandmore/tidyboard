...
func (h *PointsHandler) ListCategories(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "list categories")
		return
	}
...
func (h *PointsHandler) CreateCategory(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "create category")
		return
	}
...
func (h *PointsHandler) UpdateCategory(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "update category")
		return
	}
...
func (h *PointsHandler) ListBehaviors(w http.ResponseWriter, r *http.Request) {
	...
	if c := r.URL.Query().Get("category_id"); c != "" {
		id, err := uuid.Parse(c)
		if err != nil {
			respond.Error(w, http.StatusBadRequest, "bad_request", "invalid category_id")
			return
		}
		catID = &id
	}
	...
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "list behaviors")
		return
	}
	respond.JSON(w, http.StatusOK, rows)
}
...
func (h *PointsHandler) CreateBehavior(w http.ResponseWriter, r *http.Request) {
	...
	if req.CategoryID == uuid.Nil {
		respond.Error(w, http.StatusBadRequest, "validation_error", "category_id required")
		return
	}

	b, err := h.svc.CreateBehavior(r.Context(), householdID, req.CategoryID, req.Name, req.SuggestedPoints)
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "create behavior")
		return
	}
...
func (h *PointsHandler) UpdateBehavior(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "update behavior")
		return
	}
...
func (h *PointsHandler) Grant(w http.ResponseWriter, r *http.Request) {
	...
	if req.Points == 0 {
		respond.Error(w, http.StatusBadRequest, "validation_error", "points must be non-zero")
		return
	}
	...
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "grant")
		return
	}
...
func (h *PointsHandler) GetBalance(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "get balance")
		return
	}
...
func (h *PointsHandler) Scoreboard(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respond.Error(w, http.StatusInternalServerError, "internal_error", "scoreboard")
		return
	}
	respond.JSON(w, http.StatusOK, rows)
}
